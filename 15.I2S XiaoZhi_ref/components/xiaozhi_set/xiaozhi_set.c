...
void xiaozhi_init(void)
{
    ...
    esp_xiaozhi_chat_config_t chat_cfg = {
        ...
        .mcp_engine      = mcp,//把引擎挂给对话
        .owns_mcp_engine = true,
    };
    esp_xiaozhi_chat_init(&chat_cfg, &chat_hd);//用这个配置初始化对话
...
void xiaozhi_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        if (app.xiaozhi.xiaozhi_status == 1) {//"我要启动"请求
            esp_xiaozhi_chat_start(chat_hd);
            app.xiaozhi.xiaozhi_status = 0;
        }
        // ui_SwitchXiaoZhiSpeak 状态到了 OFF
        else if (app.xiaozhi.xiaozhi_status == 2) {//我要停止请求
            esp_xiaozhi_chat_stop(chat_hd);
            app.xiaozhi.xiaozhi_status = 0;
        }
        // 启动成功，开始对话
        else if (app.xiaozhi.xiaozhi_status == 3) {    //状态=对话中
            size_t bytes_read = 0;
            i2s_channel_read(pdm_rx_chan, pdm_buf, sizeof(pdm_buf), &bytes_read, pdMS_TO_TICKS(60));//读麦克风60ms的数据
            if (bytes_read > 0) {
                esp_xiaozhi_chat_send_audio_data(chat_hd, (const char *)pdm_buf, bytes_read);//上传
            }
        }
