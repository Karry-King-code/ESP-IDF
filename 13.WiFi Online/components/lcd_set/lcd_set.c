...
{
    if (percent > 100) percent = 100;  //最大100 防止传入大于100的值
    uint32_t duty = (uint32_t)percent * LCD_BL_DUTY_MAX / 100; //换算
    ledc_set_duty(LCD_BL_LEDC_MODE, LCD_BL_LEDC_CHANNEL, duty);//写值
    ledc_update_duty(LCD_BL_LEDC_MODE, LCD_BL_LEDC_CHANNEL);//生效
}